...
function fetchTimeline (params, opts) {
  ...
  const info = {apiCalls: 0, count: 0}
  const now = Date.now()
  let isFirstChunk = true
  let hasTweets = true
  let cursor

  const hasFetch = () => hasTweets && hasCursor()
  const hasCursor = () => isFirstChunk || cursor
  ...
  const stream = from(function (size, next) {
    ...
    fetchTweets((err, tweets) => {
      ...
      if (limitDays) {
        ...
        const diffDays = differenceInDays(now, olderTweetDate)

        if (diffDays > limitDays) {
          debug(`resizing, limit excedeed by ${diffDays} days`)
          const markDate = subDays(now, limitDays)
          tweets = tweets.filter(tweet => {
            const tweetDate = new Date(tweet.created_at)
            return tweetDate > markDate
          })
          hasTweets = false
        }
      }
